"""
Generate Qwen 2.5 1.5B Instruct BF16 reference fixture for Atenia
numerical validation (M4.6 Phase B.5).

Loads Qwen/Qwen2.5-1.5B-Instruct from a local checkpoint, runs a
forward pass with fixed input tokens, and dumps logits to JSON for
consumption by tests/qwen25_numerical_validation_test.rs.
...
Outputs:
    inputs.json           — token IDs used (mirrors TinyLlama / SmolLM2 smoke tests)
    expected_logits.json  — PyTorch BF16 reference logits, shape [1, 4, 151936]
"""
...
# Match the smoke test and the SmolLM2 / TinyLlama numerical fixtures.
...
def main():
    if len(sys.argv) != 2:
        print("Usage: generate.py /path/to/qwen2.5-1.5b-instruct/")
        sys.exit(1)

    model_dir = sys.argv[1]
    print(f"Loading Qwen 2.5 1.5B from {model_dir}...")
