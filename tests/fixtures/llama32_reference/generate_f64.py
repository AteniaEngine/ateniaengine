"""
Generate F64 reference logits for Llama 3.2 1B Instruct using
PyTorch (M4.6 Phase C.5; methodology per ADR-002 / ADR-004).
...
Memory note: Llama 3.2 1B in F64 is ~10 GB of weights (1.24B params
× 8 bytes). Plus activations and intermediate F32→F64 conversions,
peak RAM may approach 14 GB. We load directly in F64 to avoid the
...
def main():
    if len(sys.argv) != 2:
        print("Usage: generate_f64.py /path/to/llama-3.2-1b-instruct/")
        sys.exit(1)

    model_dir = sys.argv[1]
    print(f"Loading Llama 3.2 1B from {model_dir} directly in F64...")
