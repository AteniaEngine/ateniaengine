"""
Generate TinyLlama reference fixture for Atenia numerical validation.

Loads TinyLlama 1.1B Chat from a local HuggingFace checkpoint, runs a
forward pass with fixed input tokens, and dumps logits to JSON for
consumption by tests/tinyllama_numerical_validation_test.rs.
...
Outputs:
    inputs.json           — token IDs used (matches the M4.5-c smoke test)
    expected_logits.json  — PyTorch reference logits, shape [1, 4, 32000]
"""
...
# Same tokens as the M4.5-c end-to-end smoke test: BOS + 3 arbitrary IDs.
...
def main():
    if len(sys.argv) != 2:
        print("Usage: generate.py /path/to/tinyllama-1.1b/")
        sys.exit(1)

    model_dir = sys.argv[1]
    print(f"Loading TinyLlama from {model_dir}...")
    model = AutoModelForCausalLM.from_pretrained(
        model_dir,
        torch_dtype=torch.bfloat16,  # native dtype, matches checkpoint
    ...
    # Convert to F32 for serialization. Atenia loads weights as F32, so
    # the F32-cast PyTorch logits are the apples-to-apples reference.
