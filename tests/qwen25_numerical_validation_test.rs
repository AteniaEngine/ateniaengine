//! Numerical validation of Atenia's Qwen 2.5 1.5B forward against the
//! F64 mathematical ground truth (M4.6 Phase B.5; methodology per
//! ADR-002 / ADR-004).
//!
//! Following the methodology established with SmolLM2: the primary
//! assertion compares Atenia F32 against PyTorch F64 (mathematical
//! truth). PyTorch BF16 stats are reported as informative diagnostics
//! tracking how industry-default BF16 inference drifts from truth on
//! this specific model — but BF16 agreement is not a pass/fail gate.
//!
//! Run with:
//! ```powershell
//! $env:QWEN25_SAFETENSORS_PATH = "...\\model.safetensors"
//! cargo test --test qwen25_numerical_validation_test --release \
//!     -- --ignored --nocapture
//! ```
...
/// Mirrors the embedded config in `qwen25_end_to_end_test.rs`.
...
fn fixture_path(name: &str) -> PathBuf {
    PathBuf::from("tests/fixtures/qwen25_reference").join(name)
}
...
fn qwen25_atenia_matches_f64_ground_truth() {
    ...
    let path = env::var("QWEN25_SAFETENSORS_PATH")
        .expect("Set QWEN25_SAFETENSORS_PATH to Qwen 2.5 1.5B model.safetensors");

    let config = LlamaConfig::from_json_str(EMBEDDED_QWEN25_CONFIG)
        .expect("failed to parse Qwen 2.5 config");
    assert!(
        config.effective_attention_bias(),
        "Qwen 2.5 must resolve attention_bias=true via model_type"
    );
    ...
    assert_eq!(handles.param_ids.len(), 338);
    ...
    // Atenia F32 should match F64 truth within typical F32 precision
    // bounds. 0.5 is comfortable headroom over expected F32 drift of
    // ~1e-3 to ~1e-2 accumulated over 28 transformer blocks. A failure
    // here would indicate a real numerical bug, not a BF16 vs F32
    // precision tradeoff.
    assert!(
