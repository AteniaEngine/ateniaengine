...
fn build_qkv_sum_graph_with_bias() -> (
    Graph,
    usize,
    usize,
    usize,
...
fn run_mode(mode: &str) -> Grads {
    ...
    let wv_t = graph.nodes[wv_id]
        ...
        .clone();
    let bq_t = graph.nodes[bq_id]
        .output
        .as_ref()
        .expect("bq output missing")
        .clone();
    let bk_t = graph.nodes[bk_id]
        .output
        .as_ref()
        .expect("bk output missing")
        .clone();
    let bv_t = graph.nodes[bv_id]
        .output
        .as_ref()
        .expect("bv output missing")
        .clone();
