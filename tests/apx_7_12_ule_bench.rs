...
fn apx_7_12_ule_bench_print() {
    ...
    let mut g_ule = make_deep_graph(width, depth);
    let t_ule = now_ms(|| g_ule.execute(inputs));

    eprintln!(
        "[APX 7.12 ULE] t_hpge={:.3} ms | t_hls={:.3} ms | t_hlsd={:.3} ms | t_pfls={:.3} ms | t_ule={:.3} ms",
        t_hpge, t_hls, t_hlsd, t_pfls, t_ule
    );
}
