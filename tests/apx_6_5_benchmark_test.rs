...
fn matmul_6_5(a: &[f32], b: &[f32], m: usize, k: usize, n: usize) -> (Vec<f32>, u128) {
    let mut out = vec![0.0f32; m * n];
    let t0 = Instant::now();
    matmul_tiled_6_5(a, b, &mut out, m, k, n);
    let dt = t0.elapsed().as_micros();
    (out, dt)
}

fn bench_size(m: usize, k: usize, n: usize) {
    let a: Vec<f32> = (0..m * k).map(|i| (i as f32 * 0.127).sin()).collect();
    let b: Vec<f32> = (0..k * n).map(|i| (i as f32 * 0.337).cos()).collect();

    let (out_base, t_base) = baseline_matmul(&a, &b, m, k, n);
    let (out_64, t_64) = matmul_6_4(&a, &b, m, k, n);
    let (out_65, t_65) = matmul_6_5(&a, &b, m, k, n);

    for i in 0..out_base.len() {
        assert!((out_base[i] - out_64[i]).abs() < 1e-4);
        assert!((out_base[i] - out_65[i]).abs() < 1e-4);
    }

    let speedup_64 = t_base as f64 / t_64 as f64;
    let speedup_65_vs_64 = t_64 as f64 / t_65 as f64;

    println!(
        "[APX 6.5 BENCH] size={} -> baseline={}us | micro6.4={}us | micro6.5={}us | speedup_vs_base={:.2}% | speedup_vs_6.4={:.2}%",
        m,
        t_base,
        t_64,
        t_65,
        speedup_65_vs_64 * speedup_64 * 100.0,
        speedup_65_vs_64 * 100.0,
    );
}

#[test]
fn apx_6_5_benchmark() {
