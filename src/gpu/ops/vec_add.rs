...
impl VecAddOp {
    pub fn run(a: &GpuPtr, b: &GpuPtr, out: &GpuPtr, n: usize) {
        ...
        let src = r#"
        extern "C" __global__ void vec_add(float* A, float* B, float* C) {
        ...
        let ptx = compiler
            .compile(src, "vec_add_kernel", "compute_89")
            .unwrap();

        let module = match loader.load_module_from_ptx(&ptx.ptx) {
            Ok(m) => m,
            Err(CudaLoaderError::CpuFallback) => {
                // APX 12.x: entorno sin GPU usable -> early return, sin panic.
                return;
            }
            Err(e) => {
                panic!("[VEC_ADD] ModuleLoadFailed: {:?}", e);
            }
        };
