//! Legacy slice-based CUDA MatMul wrapper, moved from
//! `crate::apx4::gpu_kernels`. Preserves the pre-existing pass-through
//! that builds temporary CPU tensors and delegates to `cuda_matmul`.
//! Distinct from [`crate::gpu::ops::matmul`], which is the
//! `TensorGPU`-based real GPU op; this wrapper targets the APX 4.0
//! dispatcher that still operates on `&[f32]` slices.
