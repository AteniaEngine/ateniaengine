...
**Mixtral-8x7B = L0** (topology only, real weights not provisioned). All are
...
| **Mixtral** (`MixtralForCausalLM`) | `block_sparse_moe` | ❌ no local real weights | **EXPERIMENTAL · L0** (topology only) |
