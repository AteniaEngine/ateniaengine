...
| **PERF-3 — expert prefetch / async tier reads** (D) | Overlap next-layer expert NVMe reads with current-layer compute. | Med–large: hides read latency; compounds with PERF-2. | C5 harness parity (bit-identical results, deterministic); bench forward time. |
...
| **PERF-5 — MoE-generate instrumentation parity** (F) | Emit prefill/decode split, first-token latency, tok/s, cache hit-rate for `controlled_moe_generate` (today dense-only). Zero numerics. | Visibility → enables perf gates for PERF-2/3/4. | New timing assertions are structural only (no perf thresholds in CI); `cargo test --lib` green. |
