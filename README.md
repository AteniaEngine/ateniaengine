...
- **M3 🟡 (in progress)** — Real GPU allocation for `Tensor` behind a vendor-neutral storage abstraction. Redesigned after investigation revealed that `Tensor.device` was a logical label (data always lived in RAM). Structured in sub-milestones:
  ...
  **Known debts carried into M3-e** (documented so they are not mistaken for regressions):
  - CUDA-leaked files — the original list flagged five engine-layer files that imported `crate::cuda::*` directly. Four were resolved by a follow-up refactor that moved their bodies into `src/gpu/` and left the originals as `pub use` re-exports: `src/apx4/gpu_kernels.rs` → `src/gpu/ops/matmul_wrapper.rs`; `src/apx4_3/gpu_utils.rs` → `src/gpu/utils.rs`; `src/apx4_5/batch_matmul_cuda.rs` → `src/gpu/ops/batch_matmul_dispatch.rs`; `src/apx4_11/gpu_hooks.rs` → `src/gpu/dispatch/hooks.rs`. The fifth, `src/bin/apx_4_10_fused_linear_silu_bench.rs`, was reclassified as a legitimate exception: benchmark binaries that measure a specific kernel are expected to import vendor-specific code directly (see invariant #9 in the handoff). Separately, `src/apx4_3/gpu_executor.rs` still keeps three `crate::cuda::*` imports; refactoring it requires reworking `exec_gpu_segment` end to end and is deferred to its own milestone.
  - `FusedLinearActivationChain` has two duplicate forward handlers in `src/amg/graph.rs` with drifted input-count validation (3/4/5 vs 4/5) and neither registers a backward BackOp; consolidating them (or adding a real fused BackOp) is open work inherited from earlier milestones.
