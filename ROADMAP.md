...
- **M10 — Real velocity ✅ closed.** Drive the engine to usable speeds across the four reference brackets in the velocity matrix below. The matrix reflects what the engine measurably delivers today on the dev hardware (RTX 4070 Laptop, 8 GB VRAM, 32 GB RAM DDR5-5600, NVMe SN770) post-M10.3.1.1:
  ...
  | Mistral 7B v0.3     | 8 GB VRAM                   | **~8 s/tok** (fast) | < 3 s/tok | 🔧 hardware-limited; track β next     |
  | Llama 2 13B Chat    | 8 GB VRAM + 32 GB RAM (M8.7)| **15.12 s/tok** (fast) | < 5 s/tok | 🔧 in progress (track β: LLM.int8)    |

  Mistral 7B: M10.2.1 fast mode delivered **8.10 s/tok** on the dev box, ~1.6× lift over the M10.0 baseline of 12.85 s/tok. Llama 2 13B: **15.12 s/tok** under fast mode, 1.19× over the M10.0 baseline of 17.93 s/tok. Both still above their velocity targets — the residual gap closes via track β (LLM.int8 outlier decomposition) once that lands; M10's structural deliverable is the two-mode + per-checkpoint-certificate architecture, not the absolute speedup numbers on capacity-limited hardware.
  ...
  **Sub-milestone ledger (M10.0 → M10.3.1.1):**

  - **M10.0** (measurement) ✅ — characterised baseline s/tok across the 4 brackets; produced the velocity table above. No commit (characterisation only).
  - **M10.1** (CPU GEMV specialisation) ⚠️ attempted and reverted. Implementation green (214/214) but empirical lift 1.13× on Mistral and 1.00× on TinyLlama CPU vs the projected 5.4× — the hypothesis "matrixmultiply panel-packing dominates M=1" was wrong. Both paths were memory-bound on the strided B access. Rolled back; analysis in `bench_logs/`.
  - **M10.2.0** (TF32 Tensor Cores in certified mode) ✅ closed at commit `0d1fc4b`. One-line constant swap `CUBLAS_COMPUTE_32F` → `CUBLAS_COMPUTE_32F_FAST_TF32` engages Ada TF32 Tensor Cores on the existing F32-input GEMM. ADR-004 strict preserved 4/4 (drift 0.0097 to 0.217 across the fixture, all under 0.5 with margin 2.3× to 51×). 13B steady-state 17.30 s/tok with override stable plan.
  - **M10.2.1** (BF16-TC native fast mode) ✅ closed at commit `c750d43` + ADR-005 at `2f57cad`. New kernel `cuda_matmul_bf16_native_inplace` with `cublasGemmEx(BF16, BF16, F32)` gated by `ATENIA_FAST_MODE=1`. ADR-005 documents the per-checkpoint envelope: SmolLM2 4.7× over ADR-004 strict (worst case); Qwen 2.5 passes ADR-004 strict with margin. Llama 2 13B fast mode: **15.12 s/tok** (1.19×). Mistral 7B fast mode: **8.10 s/tok** (~1.6× over M10.0 baseline).
  - **M10.3** (numeric contract schema + per-checkpoint manifestos) ✅ closed at commit `b1c12b0` (schema + 4 manifestos + CERTIFICATION.md) + `43c8545` (README section). Schema v1.0.0 defined; reference manifestos shipped for the 4-model fixture in `docs/numcert/`; operator verification flow documented end-to-end (single `cargo test` invocation reproduces every drift number).
  - **M10.3.1.0** (manifest-driven mode selection) ✅ closed at commit `6bae12e` + data `243a67e`. `pipeline.rs::from_model_dir` reads `<model>/model.numcert.json` and resolves the global execution mode in priority `ATENIA_FAST_MODE > manifest.recommended_mode > certified`. Each of the 4 fixture models picks its manifest-recommended mode automatically on the binary path; F64 fixture regression 4/4 verde under certified.
  - **M10.3.1.1** (per-tensor dispatch) ✅ closed at commit `5d89ff4` + data `70c4403`. Schema v2.0.0 with populated `per_tensor_policy { default, overrides }` block; `glob_match` multi-`*` matcher; `WeightStore::apply_per_tensor_policy` post-load pass stamps each VRAM-resident `TensorGPU` with a precision byte; dispatcher reads `gpu.matmul_policy_byte()` per matmul. TinyLlama smoke confirmed mixed dispatch within a single model (66 fast FFN tensors + 88 certified attention tensors). 232/232 lib tests verde. Schema is stable for M11's top-10 expansion.
...
## M9 — INT8 W8A16 quantisation ⚠️ closed as **opt-in / experimental** (net speedup confirmed after post-close fixes)

Closed in five sub-phases (`76dd83c` → `f6949cf` → `4383862` → `73d4fe5` → `06ee91a` → `6686008` → `0e29a77` → close `74da82b`), then patched in three post-close commits (`997fd8c` honest cost, `d6e084c` headroom override, `0d0b4a5` two-pass plan_inner) that turned the milestone from "no production benefit" into a measurable end-to-end speedup. The infrastructure landed cleanly: full INT8 path from quantizer through `TensorStorage::CpuInt8` through CUDA kernels through loader integration through tier-planner cost model. The M9.0 microbench measured ~2× speedup over the M8.4c BF16 dispatch on the 4 dominant Llama 13B shapes.

What the first close (`74da82b`) got wrong: the `vram_cost_bytes` model returned `numel × 1` to mirror an aspirational "INT8 in VRAM" implementation that didn't ship in M9.2 (M9.0's `int8_to_bf16_in_vram` actually puts BF16 in VRAM after dequant, costing `numel × 2`). The lying cost over-promised VRAM and forced a malformed plan that ran 25.6 s/tok on the 13B smoke. The post-close commits closed both that gap and a second one (M7.2 adaptive RAM headroom over-conservation) and brought the smoke down to 18.8 s/tok — −9% vs the M8.7 baseline.

What did **not** land: ADR-004 strict (`max_abs_diff < 0.5` vs F64). Three quantisation strategies (per-channel; per-group g=128; per-group g=32) × four production models (TinyLlama 1.1B, SmolLM2 1.7B, Qwen 2.5 1.5B, Llama 3.2 1B) = **12 fixture runs, none satisfying ADR-004**. Best result: Llama 3.2 under g=128 at 0.516 (3.2% over the 0.5 gate). Worst: SmolLM2 under g=128 at 11.246 (22× over). Empirical surprises documented in [HANDOFF M9 §4](./docs/HANDOFF_APX_V20_M9.md#4-empirical-findings-the-12-run-experiment-matrix): g=32 was *worse* than g=128 on 2/4 models (likely because g=128 aligns with `head_dim ∈ {64, 128}` and g=32 cuts across head boundaries); SmolLM2 is the worst-case model across all strategies.

Headlines:

- **13B end-to-end on dev box**:
  - M8.7 baseline (no M9 flag): 20.7 s/tok
  - **M9 default**: **18.8 s/tok (−9%)** — plan ~75 VRAM / ~280 RAM / ~50 Disk
  - **M9 + `ATENIA_RAM_HEADROOM_OVERRIDE_GIB=8`**: **17.7 s/tok (−14%)** — plan ~75 VRAM / ~328 RAM / 0 Disk
- M9.0 GPU microbench: **2.01× / 1.98× / 1.97× / 1.93×** on Q/K/V/O proj, FFN gate/up, FFN down, LM head
- Storage: per-group g=128 (D90 — Q8_0-style; aligns with `head_dim`)
- Routing: 100% of `_proj.weight` tensors take the INT8 path under flag (counter-verified: 112/196/168/154 across 4 models)
- ADR-004 strict: **fail** on all 12 runs; full matrix in HANDOFF
- 214/214 lib tests verde (post-close); M9 ships **opt-in only — for the drift reason, not for the performance reason**

The path remains available for operators who need the capacity headroom and accept argmax-quality (typically 3/4 to 4/4 positions match the F64 reference) over bit-magnitude-equivalence. Default unset = bit-identical to M8.6 production. ADR-004-strict INT8 requires either reducing the cascade depth (M9.5: FFN-only mixed precision) or bypassing the outlier problem (M10: LLM.int8 / GPTQ / AWQ outlier decomposition with calibration). Four follow-up tracks (α / β / γ / δ) catalogued in [HANDOFF M9 §10](./docs/HANDOFF_APX_V20_M9.md#10-open-issues--how-to-resume).

See [HANDOFF M9](./docs/HANDOFF_APX_V20_M9.md) for the full closing notes.
...
- **Command-line interface (CLI-0 → CLI-6)** — a product-grade CLI built as a frontier layer: structured `E-*` errors with a unified exit-code scheme, a logging layer (`--quiet` / `--verbose` / `--debug` / `--trace`, `--log-file`, `--trace-id`), diagnostics (`atenia doctor` / `diagnose` / `capabilities`), an interactive `atenia chat` REPL, and a curated `atenia download` (three audited public checkpoints) for the first-model path. No runtime change. Full reference: [docs/CLI.md](./docs/CLI.md).

CI is dual-blocking (CUDA + no-CUDA), and the core no longer depends on model identity. `cargo test --lib` is at 516/516. Per-milestone history and commits live in [MILESTONES.md](./docs/MILESTONES.md); current readiness in [docs/STATUS.md](./docs/STATUS.md).
