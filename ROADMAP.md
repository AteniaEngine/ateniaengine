...
## M9 — INT8 W8A16 quantisation ⚠️ closed as **opt-in / experimental**

Closed in five sub-phases (`76dd83c` → `f6949cf` → `4383862` → `73d4fe5` → `06ee91a` → `6686008` → `0e29a77` → close commit). The infrastructure landed cleanly: full INT8 path from quantizer through `TensorStorage::CpuInt8` through CUDA kernels through loader integration through tier-planner cost model. The M9.0 microbench measured ~2× speedup over the M8.4c BF16 dispatch on the 4 dominant Llama 13B shapes. Under `ATENIA_M9_INT8=1` the planner places **167 VRAM tensors / 0 Disk** on Llama 2 13B (vs ~80 / 0 in M8 baseline) — the capacity-doubling claim from the M9.0 research is empirically validated.
...
- 13B plan under flag: **VRAM 167 / RAM 196 / Disk 0** (vram_bytes 6.98 GiB, ram_bytes 10.28 GiB)
...
- 210/210 lib tests verde; M9 ships **opt-in only**
